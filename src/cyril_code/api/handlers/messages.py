...
from cyril_code.api.detection import is_safety_classifier_request
from cyril_code.api.optimization_handlers import try_optimizations
from cyril_code.api.request_errors import (
    http_status_for_unexpected_api_exception,
    log_unexpected_api_exception,
    require_non_empty_messages,
    unexpected_http_exception,
)
from cyril_code.api.request_ids import new_request_id
...
from cyril_code.api.web_tools.egress import (
    WebFetchEgressPolicy,
    web_fetch_allowed_scheme_set,
)
from cyril_code.api.web_tools.request import (
    is_web_server_tool_request,
    unsupported_server_tool_error,
)
from cyril_code.api.web_tools.streaming import stream_web_server_tool_response
from cyril_code.application.errors import ApplicationError, InvalidRequestError
from cyril_code.application.execution import ProviderExecutor, TokenCounter
from cyril_code.application.ports import ProviderResolver
from cyril_code.application.routing import ModelRouter, RoutedMessagesRequest
...
from cyril_code.core.diagnostics import safe_exception_message
from cyril_code.core.failures import ExecutionFailure, find_execution_failure
from cyril_code.core.reasoning import ReasoningControl, ReasoningPolicy
from cyril_code.core.trace import trace_event
...
class MessagesHandler:
    ...
    async def _to_public_response(
        ...
        *,
        ...
        if not stream:
            # Non-streaming clients (e.g. Cyril Code utility calls) need a
    ...
    def _intercept_web_server_tool(
        ...
        trace_event(
            stage="routing",
            event="cyril_code.api.optimization.web_server_tool",
            source="api",
            model=routed.resolved.original_model,
        )
    ...
    def _intercept_local_optimization(
        ...
        trace_event(
            stage="routing",
            event="cyril_code.api.optimization.short_circuit",
            source="api",
            model=routed.resolved.original_model,
        )
