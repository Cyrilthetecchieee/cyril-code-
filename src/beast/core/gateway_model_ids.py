...
# Beast currently treats any model id containing ``claude-3-`` as not
...
def gateway_model_id(provider_model_ref: str) -> str:
    """Return the normal Beast-discoverable id for a provider/model ref."""
    return f"{GATEWAY_MODEL_ID_PREFIX}/{provider_model_ref}"


def no_thinking_gateway_model_id(provider_model_ref: str) -> str:
    """Return a Beast-discoverable id that disables client thinking."""
