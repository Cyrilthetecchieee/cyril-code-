...
frames = create_pulse_animation(
    object_data={"emoji": "❤️", "size": 100}, pulse_type="smooth", scale_range=(0.8, 1.2)
)
...
frames = create_crossfade(
    object1_data={"emoji": "😊", "size": 100}, object2_data={"emoji": "😂", "size": 100}
)
```
...
frames = create_zoom_animation(
    zoom_type="in", scale_range=(0.1, 2.0), add_motion_blur=True
)
...
# Wave movement
frames = create_move_animation(
    motion_type="wave", motion_params={"wave_amplitude": 50, "wave_frequency": 2}
)
...
for i in range(12):
    ...
    draw_emoji_enhanced(
        frame, "😱", position=(64 - size // 2, 64 - size // 2), size=size, shadow=False
    )
    builder.add_frame(frame)
