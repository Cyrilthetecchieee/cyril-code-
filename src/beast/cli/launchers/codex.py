"""Installed `beast-codex` launcher."""
...
from beast.cli.local_http import (
    open_local_request,
    with_local_proxy_bypass,
)
from beast.cli.proxy_auth import proxy_auth_token
from beast.config.paths import codex_model_catalog_path
from beast.config.server_urls import local_proxy_root_url
...
def launch(argv: Sequence[str] | None = None) -> None:
    """Launch Codex CLI with Beast proxy configuration."""
    ...
    if error := preflight_proxy(proxy_root_url):
        print(
            f"Beast proxy is not reachable at {proxy_root_url}: {error}",
            file=sys.stderr,
        )
        print("Start it in another terminal with: start", file=sys.stderr)
        raise SystemExit(1)
...
def build_codex_launcher_command(
    *,
    ...
    """Return a Codex command with ephemeral BEAST provider config."""
...
def codex_model_catalog_config_args(
    ...
    try:
        ...
        if not isinstance(models, list) or not models:
            print(
                "Beast warning: Codex model catalog is empty; "
        ...
        print(
            "Beast warning: could not prepare Codex model catalog "
...
def codex_config_args(*, api_url: str, model: str | None = None) -> list[str]:
    """Return Codex `-c` assignments for the ephemeral BEAST provider."""

    args = [
        "-c",
        _toml_assignment("model_provider", "fcc"),
        "-c",
        _toml_assignment("model_providers.fcc.name", "Beast"),
        "-c",
        _toml_assignment("model_providers.fcc.base_url", _ensure_v1_url(api_url)),
        "-c",
        _toml_assignment("model_providers.fcc.auth.command", "beast-codex"),
        "-c",
        _toml_assignment(
