"""Installed `cyril-codex` launcher."""
...
from cyril_code.cli.local_http import (
    open_local_request,
    with_local_proxy_bypass,
)
from cyril_code.cli.proxy_auth import proxy_auth_token
from cyril_code.config.paths import codex_model_catalog_path
...
def launch(argv: Sequence[str] | None = None) -> None:
    """Launch Codex CLI with Cyril Code proxy configuration."""
...
def build_codex_launcher_command(
    *,
    ...
    """Return a Codex command with ephemeral CYRIL provider config."""
...
def codex_model_catalog_config_args(
    ...
    try:
        ...
        if not isinstance(models, list) or not models:
            print(
                "Cyril Code warning: Codex model catalog is empty; "
        ...
        print(
            "Cyril Code warning: could not prepare Codex model catalog "
...
def codex_config_args(*, api_url: str, model: str | None = None) -> list[str]:
    """Return Codex `-c` assignments for the ephemeral CYRIL provider."""
