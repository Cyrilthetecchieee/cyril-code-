"""Platform-neutral lifecycle for the CYRIL desktop shell."""

import threading
from collections.abc import Callable
from typing import Protocol

from cyril_code.cli.commands import (
    ServerStatus,
    ServerSupervisor,
...
from cyril_code.config.paths import config_dir_path
...
class DesktopController:
    """Coordinate one tray loop with one in-process CYRIL server owner."""
    ...
    def run(self) -> None:
        """Run the tray on this thread and the CYRIL server on its owned worker."""
...
def launch_desktop(tray_factory: DesktopTrayFactory) -> None:
    """Start the singleton desktop host or focus the already running CYRIL UI."""
