...
from cyril_code.core.anthropic import anthropic_error_type_for_failure
...
from cyril_code.core.async_iterators import try_close_async_iterator
from cyril_code.core.diagnostics import safe_exception_message
from cyril_code.core.failures import find_execution_failure
from cyril_code.core.trace import close_stream_input, trace_event
    ...
    async def _close(self, *, preserved_error: BaseException | None) -> None:
        ...
        if task is None:
            task = asyncio.create_task(
                ...
                name="cyril_code-api-response-cleanup",
            )
...
def _trace_response_cleanup_failure(operation: str, exc: BaseException) -> None:
    trace_event(
        stage="egress",
        event="cyril_code.api.response.cleanup_failed",
        source="api",
        operation=operation,
...
async def _close_pre_start_body(
    ...
    *,
    ...
    task = asyncio.create_task(
        ...
        name="cyril_code-api-pre-start-stream-cleanup",
    )
    await _wait_for_cleanup(task)
