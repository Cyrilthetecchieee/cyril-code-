"""Shared filesystem paths for Cyril Code configuration."""
...
def config_dir_path() -> Path:
    ...
    return Path.home() / CYRIL_CONFIG_DIRNAME


def managed_env_path() -> Path:
    """Return the default user-managed env file path."""

    return config_dir_path() / CYRIL_ENV_FILENAME
...
def legacy_env_paths() -> tuple[Path, ...]:
    ...
    home = Path.home()
    return (
        home / LEGACY_REPO_DIRNAME / CYRIL_ENV_FILENAME,
        home / LEGACY_XDG_CONFIG_DIRNAME / LEGACY_REPO_DIRNAME / CYRIL_ENV_FILENAME,
    )
...
def server_log_path() -> Path:
    ...
    return config_dir_path() / CYRIL_LOGS_DIRNAME / SERVER_LOG_FILENAME
...
def openai_auth_path() -> Path:
    """Return CYRIL's private ChatGPT credential file path."""
