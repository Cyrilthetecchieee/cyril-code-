...
from cyril_code.api.dependencies import get_settings
...
def test_count_tokens_endpoint(client):
    ...
    with patch("cyril_code.api.routes.get_token_count", return_value=5):
        response = client.post("/v1/messages/count_tokens", json=payload)
...
def test_count_tokens_error_returns_500(client):
    ...
    with patch(
        "cyril_code.api.routes.get_token_count",
