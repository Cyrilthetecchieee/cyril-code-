...
const API_KEY_ENV = "CYRIL_PI_API_KEY";
const BASE_URL_ENV = "CYRIL_PI_BASE_URL";
...
function catalogModelIds(payload: unknown): string[] {
	if (!isRecord(payload) || payload.object !== "list" || !Array.isArray(payload.data)) {
		throw new Error("CYRIL model catalog returned an invalid response shape.");
	}
...
export function projectCyrilModels(payload: unknown): ProviderModelConfig[] {
	...
	if (models.length === 0) {
		throw new Error("CYRIL model catalog contains no routable provider models.");
	}
	return models;
}
	...
	try {
		...
		try {
			...
			if (error instanceof Error && error.name === "AbortError") {
				throw new Error(`CYRIL model catalog timed out after ${CATALOG_TIMEOUT_MS}ms.`);
			}
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Could not reach the CYRIL model catalog: ${message}`);
		}

		if (!response.ok) {
			throw new Error(`CYRIL model catalog returned HTTP ${response.status}${requestIdSuffix(response)}.`);
		}
