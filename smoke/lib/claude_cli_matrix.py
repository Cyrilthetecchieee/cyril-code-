"""Cyril Code CLI characterization helpers for provider smoke matrices."""
...
def run_claude_cli(
    *,
    ...
    """Run Cyril Code CLI against the local smoke proxy."""
...
def _has_proxy_request(log_delta: str) -> bool:
    return (
        ...
        or '"event": "cyril_code.api.request.received"' in log_delta
        or (
...
def _request_count(log_delta: str) -> int:
    ...
    structured_log_count = log_delta.count('"event": "cyril_code.api.request.received"')
    return max(access_log_count, service_log_count, structured_log_count)


def _marker(scope: str, prefix: str) -> str:
    return f"CYRIL_{scope}_{prefix}_{uuid.uuid4().hex[:8].upper()}"
