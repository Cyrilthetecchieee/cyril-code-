...
This document is a maintainer-oriented map of Cyril Code. It explains the
...
Cyril Code is a local proxy for agent clients. It accepts Anthropic
Messages traffic from Cyril Code and Pi clients and OpenAI Responses traffic
...
- CLI launchers: wrapper entrypoints prepare Cyril Code, Codex, and Pi sessions
...
```mermaid
flowchart LR
    ClaudeCode[Cyril Code CLI and Extensions] --> ProxyAPI[FastAPI Proxy]
...
- [src/cyril_code/application/](src/cyril_code/application/) is the dependency-leaf application boundary. It
...
- [src/cyril_code/api/](src/cyril_code/api/) is the HTTP adapter. It owns the FastAPI app, routes, API product
...
- [src/cyril_code/cli/](src/cyril_code/cli/) owns console entrypoints, client CLI launchers, process/session
  management, and client adapter contracts.
- [src/cyril_code/config/](src/cyril_code/config/) owns settings, provider metadata, filesystem paths,
  logging setup, constants, and provider ID catalogs.
- [src/cyril_code/core/](src/cyril_code/core/) owns provider-neutral protocol logic: wire request and response
...
- [src/cyril_code/messaging/](src/cyril_code/messaging/) owns optional platform adapters, incoming message
  handling, tree queues, transcript rendering, persistence, commands, and voice
  support.
- [src/cyril_code/providers/](src/cyril_code/providers/) owns provider construction, the shared OpenAI-chat
...
- [src/cyril_code/runtime/](src/cyril_code/runtime/) is the process composition root. It owns application
...
`cyril_code.cli.entrypoints` imports
`cyril_code.runtime.bootstrap` because the installed server executable
...
[core/version.py](src/cyril_code/core/version.py) is the sole runtime owner
of the CYRIL release version. It reads installed distribution metadata for
FastAPI/OpenAPI, CYRIL-owned CLI `--version` output, and the outbound web-tools
...
their wrapped clients except for CYRIL-owned ephemeral provider configuration.

The main ownership rule is that Anthropic and Responses protocol schemas and
shared protocol behavior belong in [src/cyril_code/core/](src/cyril_code/core/), while request routing and
provider execution belong in [src/cyril_code/application/](src/cyril_code/application/). Routes use core schemas
...
CYRIL optimizes for installed user workflows, not internal compatibility. The
...
- `start`, the Windows/macOS CYRIL Desktop shell, and the local Admin UI for
  configuring supported providers, model routing, auth, server tools, messaging,
  and diagnostics.
- `cyril_code`, Cyril Code, and the Anthropic-compatible proxy behavior Claude
...
- `cyril-codex`, Codex CLI/extensions, and the streaming OpenAI Responses behavior
...
- `cyril-pi`, Pi, and the Anthropic-compatible proxy behavior Pi relies on,
  including an CYRIL-scoped model catalog, streaming text and reasoning, and tool
  use/results.
...
- [api/handlers/](src/cyril_code/api/handlers/) owns customer-facing API product flows:
  ...
  [application/execution.py](src/cyril_code/application/execution.py) only for shared
  provider resolution, preflight, tracing, token counting, and streaming.
- [providers/openai_chat/](src/cyril_code/providers/openai_chat/) owns the common upstream provider
  ...
  protocol rules belong in [src/cyril_code/core/](src/cyril_code/core/).
- [messaging/workflow.py](src/cyril_code/messaging/workflow.py) coordinates messaging runtime
...
- [config/admin/](src/cyril_code/config/admin/) owns Admin UI config behavior. Keep
...
- `start` calls `cyril_code.cli.entrypoints:serve`.
- `cyril-desktop` is a GUI script calling
  `cyril_code.cli.desktop_entrypoint:launch` on Windows and macOS.
- `cyril_code` calls `cyril_code.cli.launchers.claude:launch`.
- `cyril-codex` calls `cyril_code.cli.launchers.codex:launch`.
- `cyril-pi` calls `cyril_code.cli.launchers.pi:launch`.
...
[cli/entrypoints.py](src/cyril_code/cli/entrypoints.py) starts the FastAPI server with Uvicorn.
...
[cli/desktop.py](src/cyril_code/cli/desktop.py) owns the platform-neutral
...
shutdown as `start`. [cli/desktop_tray.py](src/cyril_code/cli/desktop_tray.py)
owns only native status-area presentation and callbacks.

[runtime/bootstrap.py](src/cyril_code/runtime/bootstrap.py) is the single production composition function. The CLI
...
configures logging, constructs the runtime owners and the configured voice
  ...
  the consumer-owned ports in [application/ports.py](src/cyril_code/application/ports.py); Admin operations retain
  their inbound-adapter port in [api/ports.py](src/cyril_code/api/ports.py).

[api/app.py](src/cyril_code/api/app.py) registers routers and exception
...
[runtime/application.py](src/cyril_code/runtime/application.py) owns process startup and shutdown, optional messaging,
...
[runtime/asgi.py](src/cyril_code/runtime/asgi.py) drives that owner from ASGI lifespan messages and preserves
the concise startup-failure contract.

[runtime/provider_manager.py](src/cyril_code/runtime/provider_manager.py) is the only owner that constructs, publishes,
...
streaming responses bind it to CYRIL's response owner, which first closes the
...
[config/settings.py](src/cyril_code/config/settings.py) owns the flat Pydantic Settings schema:
...
source detection for startup warnings also belongs to `src/cyril_code/config/env_files.py`.
...
[config/reasoning.py](src/cyril_code/config/reasoning.py) owns the typed
configuration vocabulary. CYRIL-owned dotenv files receive one-time migrations
for retired configuration keys and values before Settings loads them. Explicit
`CYRIL_ENV_FILE` files are never rewritten and instead receive an actionable
startup warning.

[config/model_refs.py](src/cyril_code/config/model_refs.py) owns provider-prefixed model ref
...
[config/admin/](src/cyril_code/config/admin/) owns the Admin UI config manifest and
managed env writes. Provider credential, configurable base URL, proxy, and display-name
metadata is generated from [config/provider_catalog.py](src/cyril_code/config/provider_catalog.py);
...
separate modules. [api/admin_routes.py](src/cyril_code/api/admin_routes.py) exposes local-only
...
[api/routes.py](src/cyril_code/api/routes.py) exposes the public proxy routes:
...
Admin routes live beside these in [api/admin_routes.py](src/cyril_code/api/admin_routes.py).

Authentication is handled by `require_proxy_auth()` in
[api/dependencies.py](src/cyril_code/api/dependencies.py). If `ANTHROPIC_AUTH_TOKEN` is blank,
proxy auth is disabled. Otherwise CYRIL accepts exactly `Authorization: Bearer
...
opaque CYRIL request ID before routing, places it in log context and request state,
...
[api/handlers/](src/cyril_code/api/handlers/) owns the public API product flows.
...
[application/execution.py](src/cyril_code/application/execution.py). `ProviderExecutor` resolves the narrow
...
object.
[api/response_streams.py](src/cyril_code/api/response_streams.py) owns public streaming egress
commit timing. It waits for the first protocol chunk before returning a
successful CYRIL-owned `StreamingResponse`. Its explicit replay iterator owns the
...
response. Once CYRIL has finalized the failure, the response includes
`x-should-retry: false` so CYRIL retains ownership of upstream retry/recovery
...
[src/cyril_code/core/openai_responses/adapter.py](src/cyril_code/core/openai_responses/adapter.py). The adapter
...
[application/routing.py](src/cyril_code/application/routing.py) resolves incoming client model names.
It supports two forms:

- Direct provider model refs such as `nvidia_nim/nvidia/model-name`.
- Gateway model IDs decoded by [core/gateway_model_ids.py](src/cyril_code/core/gateway_model_ids.py).
...
[application/reasoning.py](src/cyril_code/application/reasoning.py) then
...
[providers/runtime/discovery.py](src/cyril_code/providers/runtime/discovery.py)
...
later catalog publication. `cyril-codex` remains an additional launch-time
...
native picker with `/model`; CYRIL does not implement a proxy-level `/models`
alias.

## Provider Architecture

Provider metadata is neutral and centralized in
[config/provider_catalog.py](src/cyril_code/config/provider_catalog.py). Each
...
[providers/runtime/](src/cyril_code/providers/runtime/) owns construction details for one
...
cleanup. [providers/runtime/factory.py](src/cyril_code/providers/runtime/factory.py)
...
[providers/admission.py](src/cyril_code/providers/admission.py) owns the
...
[application/model_metadata.py](src/cyril_code/application/model_metadata.py) owns the immutable
...
[application/ports.py](src/cyril_code/application/ports.py) defines the two provider operations consumed by request
...
[providers/base.py](src/cyril_code/providers/base.py) defines provider-internal construction and lifecycle contracts:
...
[providers/openai_chat/](src/cyril_code/providers/openai_chat/) implements the concrete
...
[providers/groq/](src/cyril_code/providers/groq/) is a specialized
...
[providers/openai_codex/](src/cyril_code/providers/openai_codex/) owns
ChatGPT subscription authentication and the Codex backend's OpenAI Responses
transport. Its process-lifetime auth manager owns CYRIL's credential file,
...
[core/openai_responses/](src/cyril_code/core/openai_responses/), which
...
[providers/google_openai/](src/cyril_code/providers/google_openai/) owns the
...
combining it with CYRIL reasoning controls fails during deterministic preflight.
...
API-key endpoint; [providers/vertex/](src/cyril_code/providers/vertex/)
...
[core/anthropic/](src/cyril_code/core/anthropic/) owns reversible
...
to Kimi's named efforts and identifies CYRIL through the upstream user agent.
...
[providers/mistral/reasoning.py](src/cyril_code/providers/mistral/reasoning.py), including its
...
downgrade: if an upstream NIM deployment rejects explicit budget control, CYRIL
...
[core/reasoning.py](src/cyril_code/core/reasoning.py) owns the immutable,
...
effort through CYRIL's single product scale: `minimal`/`low=512`, `medium=1024`,
...
2. Prefer a provider's named effort vocabulary; use CYRIL's documented numeric
...
3. Never use the output-token limit as a reasoning budget. Forward exact or
   CYRIL-mapped budgets only through documented numeric fields; otherwise translate
...
1. Add provider metadata to [config/provider_catalog.py](src/cyril_code/config/provider_catalog.py).
2. Add credentials and related settings to [config/settings.py](src/cyril_code/config/settings.py)
...
3. Let Admin UI provider credential, configurable base URL, and proxy fields come from the
   ...
   [config/admin/](src/cyril_code/config/admin/) only when the generated manifest is
   insufficient.
4. Add an `OpenAIChatProfile` under [providers/openai_chat/](src/cyril_code/providers/openai_chat/) when
...
[src/cyril_code/core/anthropic/](src/cyril_code/core/anthropic/) owns Anthropic-side protocol behavior:
...
- stream lifecycle through `src/cyril_code/core/anthropic/streaming`, including the neutral
...
[src/cyril_code/core/anthropic/streaming/](src/cyril_code/core/anthropic/streaming/). The shared layer owns the
...
[core/failures.py](src/cyril_code/core/failures.py) defines the immutable,
...
[core/diagnostics.py](src/cyril_code/core/diagnostics.py) owns bounded error
...
[providers/failure_policy.py](src/cyril_code/providers/failure_policy.py)
...
[providers/stream_recovery.py](src/cyril_code/providers/stream_recovery.py)
...
execution budget. The OpenAI SDK's internal retries remain disabled so CYRIL is
the only retry owner. `ExecutionFailure.retryable` records provider-policy
eligibility; it never tells the client to retry after CYRIL has finalized the
failure.
...
[src/cyril_code/core/openai_responses/](src/cyril_code/core/openai_responses/) owns OpenAI Responses support:
...
- streaming-only `/v1/responses` support for Codex/CYRIL workflows;
...
CYRIL accepts omitted `stream` or `stream: true`; `stream: false` is rejected with
an OpenAI-shaped client error because installed CYRIL/Codex workflows only need
...
entrypoint;
[src/cyril_code/core/openai_responses/streaming/](src/cyril_code/core/openai_responses/streaming/) owns the
...
CYRIL does not validate custom-tool grammars.
...
`encrypted_content`; the opaque value is not exposed as visible text and CYRIL
...
[api/optimization_handlers.py](src/cyril_code/api/optimization_handlers.py) short-circuits
...
Cyril Code auto-mode safety-classifier requests are a message-only routing
...
so Cyril Code receives a parser-readable `<block>yes</block>` or
`<block>no</block>` verdict.

Local `web_search` and `web_fetch` handling lives under
[api/web_tools/](src/cyril_code/api/web_tools/). When `ENABLE_WEB_SERVER_TOOLS` is true, the
Messages handler can stream local Anthropic server-tool responses without sending the
request upstream. [api/web_tools/egress.py](src/cyril_code/api/web_tools/egress.py) enforces URL
...
[cli/local_http.py](src/cyril_code/cli/local_http.py) owns the direct
agent-to-CYRIL connection boundary. Launcher health checks and local model-catalog
...
adds the configured CYRIL host and standard loopback names to both `NO_PROXY` and
`no_proxy`. Provider-specific upstream proxies remain provider-owned and do not
participate in this local boundary.

[cli/proxy_auth.py](src/cyril_code/cli/proxy_auth.py) owns the neutral
proxy-auth token policy shared by client launchers. A blank configured token
becomes the local-only `cyril_code-no-auth` sentinel so clients cross their login gates
while CYRIL continues to run without API authentication.

[cli/claude_env.py](src/cyril_code/cli/claude_env.py) owns the canonical
Cyril Code proxy environment used by every CYRIL-launched Claude process. It
...
proxy auth uses the shared local-only sentinel so Cyril Code reaches the proxy
instead of stopping at its login gate.

[cli/launchers/claude.py](src/cyril_code/cli/launchers/claude.py) owns the installed
`cyril_code` launcher:

- `cyril_code` applies the shared proxy environment without changing the user's
  Claude command arguments.

[cli/launchers/codex.py](src/cyril_code/cli/launchers/codex.py) owns the installed
`cyril-codex` launcher:

- `cyril-codex` strips official OpenAI and Codex credential variables.
...
- The server lifecycle independently keeps that same file synchronized for
  Codex App and IDE processes that are not launched through `cyril-codex`.
- Launcher-side catalog discovery authenticates directly with the canonical proxy
  token.
- Inference through `cyril-codex`, Codex App, and IDE integrations uses the same
  command-backed provider authentication. Codex invokes
  `cyril-codex --print-proxy-auth-token`, which reads the canonical CYRIL settings,
...
[cli/launchers/pi.py](src/cyril_code/cli/launchers/pi.py) owns the installed
`cyril-pi` launcher and [cli/launchers/pi_extension.ts](src/cyril_code/cli/launchers/pi_extension.ts)
is its bundled Pi adapter:

- Session commands load the extension from its absolute installed path and
  scope Pi to the ephemeral `cyril_code/**` provider, whose model IDs
  retain CYRIL's nested `provider/model` routing reference.
- The extension fetches CYRIL's `/v1/models` catalog before registration, projects
...
- CYRIL connection values live only in child-process `CYRIL_PI_*` variables. Native
...
- Pi package-management, configuration, help, and version commands pass through
  unchanged because they do not create an CYRIL-backed session.

[cli/managed/](src/cyril_code/cli/managed/) owns managed Cyril Code subprocesses used by
...
Codex and Pi are supported through their installed launchers. CYRIL does not keep
...
Messaging is optional. [runtime/application.py](src/cyril_code/runtime/application.py) calls
`create_messaging_components()` from
[messaging/platforms/factory.py](src/cyril_code/messaging/platforms/factory.py) during startup.
...
[messaging/platforms/ports.py](src/cyril_code/messaging/platforms/ports.py): a
...
Runtime adapters in
[messaging/platforms/telegram.py](src/cyril_code/messaging/platforms/telegram.py) and
[messaging/platforms/discord.py](src/cyril_code/messaging/platforms/discord.py) own SDK client
...
[messaging/platforms/telegram_inbound.py](src/cyril_code/messaging/platforms/telegram_inbound.py)
and [messaging/platforms/discord_inbound.py](src/cyril_code/messaging/platforms/discord_inbound.py).
Outbound SDK calls live in
[messaging/platforms/telegram_io.py](src/cyril_code/messaging/platforms/telegram_io.py) and
[messaging/platforms/discord_io.py](src/cyril_code/messaging/platforms/discord_io.py). Shared
delivery policy lives in [messaging/platforms/outbox.py](src/cyril_code/messaging/platforms/outbox.py),
...
[messaging/platforms/voice_flow.py](src/cyril_code/messaging/platforms/voice_flow.py), which owns
...
in the `PendingVoiceRegistry` owned by [messaging/voice.py](src/cyril_code/messaging/voice.py).
...
[messaging/workflow.py](src/cyril_code/messaging/workflow.py) contains `MessagingWorkflow`, the
...
[messaging/turn_intake.py](src/cyril_code/messaging/turn_intake.py) owns slash command dispatch,
...
[messaging/node_runner.py](src/cyril_code/messaging/node_runner.py) owns managed CLI session
...
[messaging/event_parser.py](src/cyril_code/messaging/event_parser.py) normalizes managed Claude
JSON events into low-level transcript events.
[messaging/transcript/](src/cyril_code/messaging/transcript/) owns transcript assembly and
...
[messaging/rendering/](src/cyril_code/messaging/rendering/).

[messaging/command_context.py](src/cyril_code/messaging/command_context.py) defines the typed
...
[messaging/trees/runtime.py](src/cyril_code/messaging/trees/runtime.py) contains the
...
`parent_reference_id` records the exact prompt or CYRIL status that received the
...
[messaging/trees/transitions.py](src/cyril_code/messaging/trees/transitions.py) owns frozen,
...
[messaging/trees/manager.py](src/cyril_code/messaging/trees/manager.py) is the only external
...
[messaging/trees/repository.py](src/cyril_code/messaging/trees/repository.py)
is manager-private and owns only aggregate/reference indexes.
[messaging/trees/processor.py](src/cyril_code/messaging/trees/processor.py) owns every
...
[messaging/trees/node.py](src/cyril_code/messaging/trees/node.py) owns
...
[messaging/trees/graph.py](src/cyril_code/messaging/trees/graph.py) owns parent/child and
status-message lookup state, and
[messaging/trees/snapshot.py](src/cyril_code/messaging/trees/snapshot.py) owns typed persisted
...
[messaging/session/](src/cyril_code/messaging/session/) persists typed conversation snapshots
...
[messaging/session/persistence.py](src/cyril_code/messaging/session/persistence.py). One writer
...
ownership lives in
[messaging/session/managed_message_log.py](src/cyril_code/messaging/session/managed_message_log.py).
The registry accepts managed inbound prompts, voice notes, and commands as well
as CYRIL output. It migrates legacy `message_log` entries and persists the final
...
`/clear` commits CYRIL state cleanup first and then best-effort deletes the exact
authorized message-ID set through the list-based outbound port. Standalone clear
deletes every tracked user and CYRIL message in its chat; reply clear deletes only
...
missing messages; such failures never restore cleared CYRIL state.
...
[core/trace.py](src/cyril_code/core/trace.py) emits structured trace events across stages such
...
1. Add or expose the setting in [config/settings.py](src/cyril_code/config/settings.py).
2. Add the template key to [.env.example](.env.example) if users configure it.
3. Add a `ConfigFieldSpec` under [config/admin/](src/cyril_code/config/admin/), or add
...
1. For an installed wrapper, add or update a launcher under
   [cli/launchers/](src/cyril_code/cli/launchers/) and keep credential stripping local to that
   client.
2. For messaging-managed execution, update [cli/managed/](src/cyril_code/cli/managed/) only
...
3. Ensure managed task parsing emits the event shapes expected by
   [messaging/event_parser.py](src/cyril_code/messaging/event_parser.py) and
   [messaging/node_event_pipeline.py](src/cyril_code/messaging/node_event_pipeline.py).
...
1. Implement a `MessagingRuntime`, `OutboundMessenger`, and inbound normalizer
   under [messaging/platforms/](src/cyril_code/messaging/platforms/).
2. Reuse [messaging/platforms/outbox.py](src/cyril_code/messaging/platforms/outbox.py) for
   queued outbound delivery and
   [messaging/platforms/voice_flow.py](src/cyril_code/messaging/platforms/voice_flow.py) for
   voice-note handoff when the platform supports audio.
3. Add construction logic to
   [messaging/platforms/factory.py](src/cyril_code/messaging/platforms/factory.py).
4. Add settings and admin fields for tokens, allowlists, and platform-specific
   runtime options.
5. Add rendering profile support in
   [messaging/rendering/profiles.py](src/cyril_code/messaging/rendering/profiles.py) if needed.
6. Add deterministic runtime/outbound/workflow tests and optional live smoke
   targets.

### Add Protocol Behavior

1. Put shared Anthropic behavior under [src/cyril_code/core/anthropic/](src/cyril_code/core/anthropic/).
2. Put OpenAI Responses behavior under
   [src/cyril_code/core/openai_responses/](src/cyril_code/core/openai_responses/).
