"""Implementations for installed Cyril Code commands."""
...
from cyril_code.cli.launchers.common import preflight_proxy
from cyril_code.cli.process_registry import kill_all_best_effort
from cyril_code.config.env_migrations import (
    explicit_env_file_migration_warning,
    migrate_owned_env_files,
)
from cyril_code.config.paths import (
    legacy_env_paths,
    managed_env_path,
)
...
from cyril_code.runtime.bootstrap import build_asgi_app
...
class ServerSupervisor:
    """Own one CYRIL server lifecycle, including config-driven restarts."""
    ...
    def run(self, *, open_admin_browser: bool | None = None) -> None:
        ...
        with self._lock:
            self._run_scheduled = False
            if self._running:
                raise RuntimeError("The CYRIL server supervisor is already running.")
...
def schedule_open_admin_browser(settings: Settings) -> None:
    ...
    threading.Thread(
        ...
        args=(settings,),
        name="cyril_code-open-admin-browser",
        daemon=True,
    ).start()
