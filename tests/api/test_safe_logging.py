...
from beast.api import request_errors
from beast.api.handlers import MessagesHandler, TokenCountHandler
from beast.application import execution
...
def test_stream_ledger_default_debug_has_no_serialized_json_content():
    with patch("beast.core.anthropic.streaming.emitter.logger.debug") as mock_debug:
...
def test_parse_cli_event_error_logs_metadata_by_default():
    ...
    with patch("beast.messaging.event_parser.logger.info") as log_info:
        parse_cli_event(
...
def test_count_tokens_unexpected_error_default_logs_exclude_exception_text():
    ...
    from beast.core.anthropic.models import TokenCountRequest
