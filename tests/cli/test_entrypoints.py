...
def test_legacy_env_migration_supports_xdg_path(tmp_path: Path) -> None:
    """Server startup preserves config from ~/.config/cyril_code/.env."""
    from cyril_code.cli.commands import _migrate_legacy_env_if_missing

    legacy_env = tmp_path / ".config" / "cyril_code" / ".env"
...
def test_cli_scripts_are_registered() -> None:
    ...
    assert pyproject["project"]["scripts"] == {
        "start": "cyril_code.cli.entrypoints:serve",
        "cyril": "cyril_code.cli.launchers.claude:launch",
        "cyril-codex": "cyril_code.cli.launchers.codex:launch",
        "cyril-pi": "cyril_code.cli.launchers.pi:launch",
    }
    assert pyproject["project"]["gui-scripts"] == {
        "cyril-desktop": "cyril_code.cli.desktop_entrypoint:launch",
    }
...
def test_cyril_server_reports_version_without_side_effects(
    argv: tuple[str, ...],
    capsys: pytest.CaptureFixture[str],
) -> None:
    from cyril_code.cli import entrypoints

    with patch.object(entrypoints, "package_version", return_value="9.8.7"):
        entrypoints.serve(argv)

    assert capsys.readouterr() == ("cyril_code 9.8.7\n", "")
...
def test_version_entrypoint_does_not_import_command_runtime() -> None:
    script = "\n".join(
        (
            "import json",
            "import sys",
            "from cyril_code.cli.entrypoints import serve",
            "serve(['--version'])",
...
def test_non_version_entrypoint_delegates_to_server_command() -> None:
    from cyril_code.cli import commands, entrypoints
...
def test_schedule_open_admin_browser_opens_when_health_ready() -> None:
    ...
    from cyril_code.cli import commands
...
def test_config_env_migration_warns_for_explicit_env_file(
    ...
    with patch.dict(commands.os.environ, {"CYRIL_ENV_FILE": str(explicit)}):
...
def test_launch_claude_passes_args_and_child_env(
    ...
    from cyril_code.cli.launchers.claude import launch
    ...
    with (
        patch("cyril_code.cli.launchers.claude.get_settings", return_value=settings),
        patch("cyril_code.cli.launchers.claude.preflight_proxy", return_value=None),
        patch(
            "cyril_code.cli.launchers.common.shutil.which",
            return_value="resolved-claude.cmd",
        ),
        patch("cyril_code.cli.launchers.common.subprocess.Popen") as popen,
        patch("cyril_code.cli.launchers.common.register_pid") as register_pid,
        patch("cyril_code.cli.launchers.common.unregister_pid") as unregister_pid,
...
def test_launch_codex_passes_responses_config_and_child_env(
    ...
    from cyril_code.cli.launchers.codex import launch
    ...
    with (
        patch("cyril_code.cli.launchers.codex.get_settings", return_value=settings),
        patch("cyril_code.cli.launchers.codex.preflight_proxy", return_value=None),
        patch(
            "cyril_code.cli.launchers.common.shutil.which",
            return_value="resolved-codex.cmd",
        ),
        patch(
            "cyril_code.cli.launchers.codex.codex_model_catalog_path",
            return_value=catalog_path,
        ),
        patch(
            "cyril_code.cli.launchers.codex.open_local_request",
    ...
    child_env = popen.call_args.kwargs["env"]
    assert "CYRIL_CODEX_API_KEY" not in child_env
...
def test_codex_proxy_auth_command_prints_only_current_token(
    ...
    from cyril_code.cli.launchers import codex
...
def test_launch_codex_catalog_failure_warns_and_continues(
    ...
    with (
        ...
        patch("cyril_code.cli.launchers.common.subprocess.Popen") as popen,
        patch("cyril_code.cli.launchers.common.register_pid"),
        patch("cyril_code.cli.launchers.common.unregister_pid"),
        pytest.raises(SystemExit) as exc_info,
    ):
        process = popen.return_value
        process.pid = 12345
...
def test_pi_launcher_builds_scoped_session_command_and_proxy_env(
    ...
    from cyril_code.cli.launchers.pi import (
    ...
    env = build_pi_launcher_env(
        ...
        base_env={
            ...
            "CYRIL_PI_API_KEY": "stale-key",
            "CYRIL_PI_BASE_URL": "https://stale.invalid",
        },
    )
    ...
    assert env == {
        ...
        "no_proxy": "127.0.0.1,localhost,::1",
        "CYRIL_PI_BASE_URL": "http://127.0.0.1:9191",
        "CYRIL_PI_API_KEY": "proxy-token",
    }


def test_pi_launcher_uses_no_auth_sentinel_for_blank_token() -> None:
    from cyril_code.cli.launchers.pi import build_pi_launcher_env
    ...
    assert env["CYRIL_PI_API_KEY"] == "cyril_code-no-auth"
...
def test_launch_pi_registers_bundled_extension_for_sessions(
    ...
    from cyril_code.cli.launchers.pi import launch

    monkeypatch.setenv("KEEP_ME", "yes")
    monkeypatch.setenv("CYRIL_PI_API_KEY", "stale-key")
    extension = tmp_path / "pi_extension.ts"
    extension.write_text("export default () => {};", encoding="utf-8")
    settings = _launcher_settings(port=9191, token="proxy-token")

    with (
        patch("cyril_code.cli.launchers.pi.get_settings", return_value=settings),
        patch("cyril_code.cli.launchers.pi.preflight_proxy", return_value=None),
        patch(
            "cyril_code.cli.launchers.pi.pi_extension_path",
        ...
        patch(
            "cyril_code.cli.launchers.pi.pi_binary_is_compatible",
            return_value=True,
        ),
        patch("cyril_code.cli.launchers.common.subprocess.Popen") as popen,
        patch("cyril_code.cli.launchers.common.register_pid"),
    ...
    child_env = popen.call_args.kwargs["env"]
    assert child_env["CYRIL_PI_BASE_URL"] == "http://127.0.0.1:9191"
    assert child_env["CYRIL_PI_API_KEY"] == "proxy-token"
    assert child_env["NO_PROXY"] == "127.0.0.1,localhost,::1"
    assert child_env["no_proxy"] == child_env["NO_PROXY"]
    assert child_env["KEEP_ME"] == "yes"
...
def test_launch_pi_passes_management_commands_through_without_proxy(
    ...
    with (
        patch("cyril_code.cli.launchers.pi.get_settings") as get_settings,
        patch("cyril_code.cli.launchers.pi.preflight_proxy") as preflight,
        patch(
            "cyril_code.cli.launchers.common.shutil.which",
...
def test_pi_install_hints_use_official_platform_installers() -> None:
    from cyril_code.cli.launchers.pi import pi_install_hint
...
def test_pi_binary_compatibility_requires_both_launcher_capabilities(
    ...
    from cyril_code.cli.launchers.pi import pi_binary_is_compatible

    with patch(
        "cyril_code.cli.launchers.pi.subprocess.run",
...
def test_launch_claude_keyboard_interrupt_kills_child_tree() -> None:
    ...
    with (
        ...
        patch("cyril_code.cli.launchers.common.subprocess.Popen") as popen,
        patch("cyril_code.cli.launchers.common.register_pid"),
        patch("cyril_code.cli.launchers.common.kill_pid_tree_best_effort") as kill_tree,
        patch("cyril_code.cli.launchers.common.unregister_pid") as unregister_pid,
...
def test_launch_claude_exits_when_command_cannot_be_resolved(
    ...
    with (
        patch("cyril_code.cli.launchers.claude.get_settings", return_value=settings),
        patch("cyril_code.cli.launchers.claude.preflight_proxy", return_value=None),
        patch("cyril_code.cli.launchers.common.shutil.which", return_value=None),
        patch("cyril_code.cli.launchers.common.subprocess.Popen") as popen,
        pytest.raises(SystemExit) as exc_info,
    ):
        launch([])
    ...
    popen.assert_not_called()
    captured = capsys.readouterr()
    assert "Could not find Cyril Code command: claude" in captured.err
...
def test_launch_claude_unreachable_proxy_exits_with_hint(
    ...
    with (
        patch("cyril_code.cli.launchers.claude.get_settings", return_value=settings),
        patch(
            "cyril_code.cli.launchers.claude.preflight_proxy",
