...
from cyril_code.messaging.session.persistence import DebouncedJsonPersistence
from cyril_code.messaging.trees import TreeIdentity, TreeSnapshot
...
class TestSessionStoreSaveEdgeCases:
    ...
    def test_explicit_flush_reports_io_error_and_keeps_store_dirty(self, tmp_store):
        ...
        with (
            patch(
                "cyril_code.messaging.session.persistence.os.replace",
...
class TestSessionStoreTreeSnapshots:
    def test_unscoped_tree_without_legacy_ingress_is_reported_and_skipped(
        ...
        with patch("cyril_code.messaging.trees.snapshot.logger.warning") as warning:
