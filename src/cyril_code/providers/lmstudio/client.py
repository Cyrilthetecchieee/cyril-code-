...
the newer ``/v1/messages`` path renders Cyril Code conversations through the
...
``/v1/chat/completions`` path is LM Studio's mature parsing route, and cyril's
...
from cyril_code.core.anthropic import ReasoningReplayMode, get_token_count
