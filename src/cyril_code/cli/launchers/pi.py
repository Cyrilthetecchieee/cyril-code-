...
from cyril_code.cli.local_http import with_local_proxy_bypass
from cyril_code.cli.proxy_auth import proxy_auth_token
from cyril_code.config.server_urls import local_proxy_root_url
from cyril_code.config.settings import get_settings

from .common import preflight_proxy, resolve_client_binary, run_client_process

_API_KEY_ENV = "CYRIL_PI_API_KEY"
_BASE_URL_ENV = "CYRIL_PI_BASE_URL"
_BINARY_NAME = "pi"
_DISPLAY_NAME = "Pi"
_HELP_TIMEOUT_SECONDS = 5.0
_MODEL_SCOPE = "cyril_code/**"
...
def launch(argv: Sequence[str] | None = None) -> None:
    """Launch Pi with a process-local Cyril Code provider."""
    ...
    if error := preflight_proxy(proxy_root_url):
        print(
            f"Cyril Code proxy is not reachable at {proxy_root_url}: {error}",
            file=sys.stderr,
        )
    ...
    if not extension_path.is_file():
        print(
            "Cyril Code's bundled Pi extension is missing. Reinstall CYRIL.",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
def build_pi_launcher_command(
    *,
    ...
    """Return a Pi session command with ephemeral CYRIL provider registration."""
...
def build_pi_launcher_env(
    *,
    ...
    """Return a Pi environment containing only CYRIL-owned proxy variables."""

    env = with_local_proxy_bypass(
        {
            key: value
            for key, value in base_env.items()
            if not key.startswith("CYRIL_PI_")
        },
        proxy_root_url=proxy_root_url,
    )
...
def pi_binary_is_compatible(binary_path: str) -> bool:
    """Return whether an executable exposes the Pi features CYRIL requires."""
