...
def test_workflow_owns_one_idempotent_triage_state() -> None:
    ...
    assert "needs-cyril-version" in workflow
    assert "<!-- cyril-version-validator -->" in workflow
...
const bodyFor = (value) => `### CYRIL version\n\n${value}\n\n### CLI\n\nCyril Code`;
...
await run(github, context);
await run(github, context);
liveIssue.body = bodyFor("cyril_code 17.23.455");
await run(github, context);
...
comments.push({
  id: 102,
  user: { login: "github-actions[bot]" },
  body: "<!-- cyril-version-outdated -->\nstale",
});
liveIssue.labels = [{ name: "needs-cyril-version" }];
...
"""
    ...
    assert "`17.23.454`" in next(
        ...
        and "cyril-version-outdated" in call["args"]["body"]
    )
