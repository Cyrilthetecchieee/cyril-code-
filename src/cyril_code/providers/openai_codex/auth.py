"""CYRIL-owned ChatGPT credential lifecycle."""
...
from cyril_code.config.paths import openai_auth_lock_path, openai_auth_path
...
class OpenAIAuthManager:
    ...
    async def disconnect(self) -> ConnectedAccountStatus:
        """Best-effort revoke, then always remove CYRIL-owned credentials."""
        ...
        async with self._state_lock:
            ...
            if credentials is None:
                raise OpenAIReconnectRequired(
                    "Connect an OpenAI ChatGPT account in the CYRIL Admin UI."
                )
            if force_refresh:
    ...
    async def _refresh_locked(self, current: _Credentials) -> _Credentials:
        ...
        if not acquired:
            raise OpenAIReconnectRequired(
                "Timed out waiting for another CYRIL process to refresh OpenAI."
            )
        try:
            reloaded = await asyncio.to_thread(self._read_credentials)
            if reloaded is not None and (
