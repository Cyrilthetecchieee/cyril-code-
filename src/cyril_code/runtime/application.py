...
import cyril_code.cli.managed as cli_managed
import cyril_code.messaging.session as messaging_session
import cyril_code.messaging.workflow as messaging_workflow_module
from cyril_code.application.connected_accounts import (
    ConnectedAccountLoginMode,
    ConnectedAccountPort,
    ConnectedAccountStatus,
)
from cyril_code.application.errors import ApplicationUnavailableError
from cyril_code.application.model_metadata import ProviderModelRefreshResult
from cyril_code.application.ports import StopResult
from cyril_code.config.admin.persistence import (
    PreparedAdminUpdate,
    commit_prepared_admin_update,
    prepare_admin_update,
)
from cyril_code.config.admin.status import provider_config_status
from cyril_code.config.admin.values import load_value_state
from cyril_code.config.env_files import (
    ANTHROPIC_AUTH_TOKEN_ENV,
    process_env_key_is_effective,
)
from cyril_code.config.model_refs import parse_provider_type
from cyril_code.config.paths import messaging_state_dir_path
from cyril_code.config.server_urls import local_admin_url, local_proxy_root_url
from cyril_code.config.settings import Settings, get_settings
from cyril_code.messaging.platforms import factory as messaging_platform_factory
from cyril_code.messaging.platforms.factory import MessagingPlatformOptions
from cyril_code.messaging.platforms.ports import (
    MessagingPlatformComponents,
    MessagingRuntime,
)
from cyril_code.messaging.voice import Transcriber
...
class ApplicationRuntime:
    ...
    async def start(self) -> None:
        if self._started:
            return
        logger.info("Starting Cyril Code Proxy...")
        try:
